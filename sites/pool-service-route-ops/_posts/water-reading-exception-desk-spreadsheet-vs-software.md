---
title: "Water Reading Exception Desk vs. a Spreadsheet: When Software Is Worth It"
...
A spreadsheet is often the right first implementation for pool service water chemistry exception tracking. It is cheap, editable, and forces the team to define the workflow. The question is not whether spreadsheets are good or bad; it is when coordination costs become larger than the flexibility is worth.
...
- One owner can reliably manage capture readings and pool conditions.
- One owner can reliably manage validate the measurement and recent history.
- One owner can reliably manage select the approved response path.
...
Before migrating, recreate ten current records using the candidate tool. Confirm that it supports these fields without awkward workarounds: Customer pool and route stop, Reading time method and technician, Measured values and expected range, Recent treatment and weather context, Observed equipment or water condition, Approved action and chemical amount, Customer restriction or notice, Recheck result owner and time. Then walk one exception from start to finish. Test exports and deletion before importing the full history.
