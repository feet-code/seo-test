---
title: "Optometry Patient Recall Outreach Checklist for Independent Optometry Practices And Optical Dispensaries"
...
- Update Open the recall record from a verified source
...
- Update Verify the outcome and close or reschedule the recall record
...
- [ ] Review records where a new recall record is created or its due window changes
...
- [ ] Check for treating a message or scheduled task as completion of the recall record
...
Choose one metric before the next cycle. Good options for this workflow are Recall Record ready rate, Open exception age, Repeat exception rate. A checklist that never changes a metric or prevents a known failure mode is probably administrative overhead.
