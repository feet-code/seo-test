---
title: "How to Automate Optometry Patient Recall Outreach Without Losing Judgment"
...
Automation for optometry patient recall outreach should remove predictable coordination while preserving judgment for exceptions. Start from the workflow, not from a list of integrations. For independent optometry practices and optical dispensaries, the target outcome is **every due recall receives policy-aligned outreach and ends in a booking, future date, transfer, or documented stop reason**.
...
|---|---|---|
| a new recall record is created or its due window changes | Queue or prompt: Collect the required inputs and operating evidence | The risk is treating a message or scheduled task as completion of the recall record |
...
Track Recall Record ready rate, Open exception age, Repeat exception rate. Also record overrides and incorrect actions. Time saved is not useful if the process creates confusing communication or hides blocked work.
