---
title: "Physical Therapy Referral And Authorization Paperwork Checklist for Independent Physical Therapy Clinics And Small Rehabilitation Practices"
...
A checklist for physical therapy referral and authorization paperwork should prevent missing decisions, not merely prove that somebody clicked boxes. The checklist below is designed for independent physical therapy clinics and small rehabilitation practices and centers on one result: **every scheduled evaluation has its clinic-defined administrative prerequisites complete or an owned exception**.

## Before the work starts

- Confirm Referral Record identifier and source
...
- Update Open the referral record from a verified source
...
- Update Verify the outcome and close or reschedule the referral record
...
- [ ] Review records where a new referral record is created or its due window changes
...
- [ ] Check for treating a message or scheduled task as completion of the referral record
...
Choose one metric before the next cycle. Good options for this workflow are Referral Record ready rate, Open exception age, Repeat exception rate. A checklist that never changes a metric or prevents a known failure mode is probably administrative overhead.
