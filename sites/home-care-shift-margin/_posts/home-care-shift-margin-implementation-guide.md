---
title: "How to Improve Non-medical Home Care Shift and Client Profitability: A Practical Implementation Guide"
excerpt: "A step-by-step implementation guide for caregiver shift margin calculator, with fields, rules, and exceptions."
...
Agencies accept schedules whose travel and overtime patterns erase the apparent bill-pay spread. For non-medical home-care agencies, the useful goal is not to add another dashboard. It is to create a small, visible process that produces this outcome: **Flags unprofitable schedules before assignment and supports contract or schedule changes without giving care advice**.

The narrow product hypothesis is: Calculates client and shift margin from bill rate, caregiver pay, overtime, travel, minimums, cancellations, and unfilled risk. Keep the first implementation focused on the named economic decision rather than expanding it into a general operations suite.
...
### 1. Collect the minimum source records needed to evaluate non-medical home care shift and client profitability.
...
Record **revenue or avoided-loss amount** at this point, name the person responsible, and define the evidence that allows the work to move to the next step. The exit condition should be observable: either the team can let the agency owner, scheduler, or finance lead approve an action and compare the eventual result with the forecast., or the record remains open with a reason and next action.

### 4. Let the agency owner, scheduler, or finance lead approve an action and compare the eventual result with the forecast.
