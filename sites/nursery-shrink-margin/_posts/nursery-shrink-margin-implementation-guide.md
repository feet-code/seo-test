---
title: "How to Improve Wholesale Nursery Inventory Shrink and Order Profitability: A Practical Implementation Guide"
excerpt: "A step-by-step implementation guide for plant nursery shrink tracking, with fields, rules, and exceptions."
...
Biological shrink and substitutions make nominal catalog margin unreliable by the time plants ship. For wholesale plant nurseries and growers, the useful goal is not to add another dashboard. It is to create a small, visible process that produces this outcome: **Flags low-margin orders and aging inventory actions using current sellable quantity and expected loss**.

The narrow product hypothesis is: Combines crop cost, availability, shrink, substitutions, freight, customer price, and aging to protect order and SKU margin. Keep the first implementation focused on the named economic decision rather than expanding it into a general operations suite.
...
### 1. Collect the minimum source records needed to evaluate wholesale nursery inventory shrink and order profitability.
...
Record **revenue or avoided-loss amount** at this point, name the person responsible, and define the evidence that allows the work to move to the next step. The exit condition should be observable: either the team can let the owner, sales manager, or inventory manager approve an action and compare the eventual result with the forecast., or the record remains open with a reason and next action.

### 4. Let the owner, sales manager, or inventory manager approve an action and compare the eventual result with the forecast.
