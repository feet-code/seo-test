---
title: "Wholesale Nursery Inventory Shrink and Order Profitability Calculator: Inputs, Formula, and Decisions"
excerpt: "A practical decision model for nursery order profitability, including inputs, calculations, and actions."
...
Biological shrink and substitutions make nominal catalog margin unreliable by the time plants ship. A useful calculation for wholesale nursery inventory shrink and order profitability should help wholesale plant nurseries and growers make a specific margin decision and produce this outcome: **Flags low-margin orders and aging inventory actions using current sellable quantity and expected loss**.

The product hypothesis is: Combines crop cost, availability, shrink, substitutions, freight, customer price, and aging to protect order and SKU margin. The model should expose its assumptions and compare the eventual result with the forecast. Avoid universal benchmarks because volume, service model, cost structure, and exception mix differ. Establish a baseline from the buyer's own records and compare the decision process against itself.

## Start with the economic question

Perishable inventory and recurring wholesale orders create measurable savings and a defensible operational dataset. Write that question at the top of the model. If an input cannot change the recommendation or explain the result, keep it out of the first version.
