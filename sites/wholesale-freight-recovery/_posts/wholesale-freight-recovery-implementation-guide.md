---
title: "How to Improve Wholesale Freight Cost Recovery and Delivery Pricing: A Practical Implementation Guide"
excerpt: "A step-by-step implementation guide for customer freight charge audit, with fields, rules, and exceptions."
...
Freight policies and overrides leave orders below target margin even when product pricing is correct. For specialty wholesalers that deliver or ship customer orders, the useful goal is not to add another dashboard. It is to create a small, visible process that produces this outcome: **Finds under-recovered freight and models customer, order-minimum, or zone policy changes**.

The narrow product hypothesis is: Compares actual freight, delivery, fuel, liftgate, and accessorial cost with customer charges and free-freight rules. Keep the first implementation focused on the named economic decision rather than expanding it into a general operations suite.
...
### 1. Collect the minimum source records needed to evaluate wholesale freight cost recovery and delivery pricing.
...
Record **revenue or avoided-loss amount** at this point, name the person responsible, and define the evidence that allows the work to move to the next step. The exit condition should be observable: either the team can let the controller, operations director, or pricing manager approve an action and compare the eventual result with the forecast., or the record remains open with a reason and next action.

### 4. Let the controller, operations director, or pricing manager approve an action and compare the eventual result with the forecast.
