---
title: "Cemetery Interment Readiness Tracking Template: Fields, Statuses, and Rules"
...
The most useful cemetery interment readiness tracking template is a small operating record. It should answer what is happening, who owns it, what evidence exists, and when the next decision occurs. This structure works in a spreadsheet, database, or focused application.

## Recommended record fields

| Field | Why it exists | Update point |
|---|---|---|
| Interment Case identifier and source | Prevents the record from depending on memory or an inbox search | Open the interment case from a verified source |
...
| Exception category impact and decision boundary | Prevents the record from depending on memory or an inbox search | Verify the outcome and close or reschedule the interment case |
| Owner next action and responsible reviewer | Prevents the record from depending on memory or an inbox search | Open the interment case from a verified source |
...
Use workflow statuses that describe reality: **Open The Interment Case From A Verified Source → Collect The Required Inputs And Operating Evidence → Validate Readiness And Classify Material Exceptions → Assign The Next Action And Communicate The Decision → Verify The Outcome And Close Or Reschedule The Interment Case**. Add **Waiting** only when you also capture a waiting reason and review date. Add **Closed—Not Completed** when an item legitimately ends without the desired outcome.

## Follow-up rules

- When a new interment case is created or its due window changes, assign a next action and review date.
...
- A lot ownership record needs clarification
- Weather changes equipment access
- The service time changes after the opening crew is assigned

For each example, write the current status, next action, owner, and supporting evidence. This makes the template testable with real work rather than idealized sample data.

## Quality-control rules

- Every open interment case needs one owner and a next review time
- Completion requires recorded evidence that every interment is released only after the cemetery's authority, location, scheduling, coordination, and site checks are complete
